...
class Thorn extends Client {
  constructor(options = {}) {
    ...
    console.log(
      gradient.passion(
        figlet.textSync(`Thorn v${require("../package.json").version}`)
      )
    );
  }
  ...
  loadLanguages() {
    for (const language of glob.sync(`${this.directory}/Languages/**/*.js`)) {
      ...
      if (!this.language[ISO]) this.language[ISO] = {};
  ...
  loadInteractions() {
    for (const interactions of glob.sync(
      `${this.directory}/Interactions/*.js`
    )) {
      delete require.cache[interactions];
      const file = new (require(path.resolve(interactions)))(this);

      if (!(file instanceof Interaction)) return;
