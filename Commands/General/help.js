...
class Help extends Command {
    ...
    async run( message, [ command ] ) {
        ...
        if ( command ) {
            ...
            super.respond( { embed: {
                color       : "RED",
                title       : `${emojis.scrolls} ${this.language.pTitle( commandData.name )}`,
        ...
        const pagination = await message.channel.send({
            embed: {
                color       : "RED",
                title       : `${emojis.scrolls} ${this.language.firstTitle}`,
                thumbnail   : {
                    url     : `attachment://${face}`
                },
        ...
        collector.on( "collect", async ( button ) => {
            ...
            await pagination.edit({
                embed: {
                    ...
                    title       : `${emojis.scrolls} ${button.id} - Scroll`,
                    description : [
                        "",
                        categories[button.id].map( ( cmd ) => {
                            return `\`${message.guild.prefix}${cmd.name}\` - ${cmd.description}`
                        }).join( "\n" ),
                         "",
                    ].join( "\n" ),
                    footer      : {
