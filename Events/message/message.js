...
class message extends Event {
    ...
    async run( message ) {
        ...
        if ( message.guild) {
            ...
            if (message.guild.plugins?.experience?.enabled) {
        ...
        if ( message.guild && message.guild.disabledCommands?.length && message.guild.disabledCommands.includes( cmd ) )
            return;

        // —— If the command can only be executed by the master
        if ( command.ownerOnly && !client.config.Master.includes(message.author.id))
        ...
        if ( message.guild ) {
            ...
            // Premium Commands
        if ( command.premium && !(await premiumSchema.findOne({ _ID: message.author.id }))) return message.reply("Upgrade to Premium to use this command")
